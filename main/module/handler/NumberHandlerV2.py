...
class NumberHandlerV2:
    def __init__(self, model):
        self.model = model
        self.figure_handler = None

    def get_number_with_model(self, image, device='cpu'):
        """
        ...
        """
        device를 cuda로도 할 수 있게 파라미터로 받게 했음.
        """
        ...
        """
        image = image.astype('float32') / 255.0 수정: 255로 나누면 자동으로 float32로 바뀌므로 불필요한 연산인 듯함.
        """

        """
        torchvision.transforms 선언 제거: 학습시킬 때 정규화하지 않았기 때문에 굳이 transforms 인스턴스를 생성해 메모리를 낭비할 필요가 없음.
        """
        ...
        """
        image의 구조는 (batch_size, channels, h, w)으로 되어 있고 이진마스크의 chanel 수는 1이고 배치의 크기도 1이기 때문에 image의 크기를 변경한다.
        """
        ...
        with torch.no_grad():
            ...
            """
            _, predicted = torch.max(outputs, dim = 1) <- 이렇게 쓰는 것보다는 수정한 것처럼 쓰는 것이 더 편해서 수정함 ;).
            """
